...
void Bone::Render() {
	...
	Renderer::Instance().RotateMatrix(this->currentRotation);
	if (this->image != nullptr){
		image->SetHandle(this->handleX * image->GetWidth(), this->handleY * image->GetHeight());
		Renderer::Instance().DrawImage(this->image, 0, 0, 0, this->currentScaleX * image->GetWidth(), this->currentScaleY * image->GetHeight());
		Renderer::Instance().TranslateMatrix(this->pivotX * image->GetWidth(), this->pivotY * image->GetHeight(), 0);
	}
	for (uint32 i = 0; i < children.Size(); i++){
		children[i].Render();
	}
...
double Bone::Interpolate(int32 id, int32 prevId, int32 nextId, double prevVal, double nextVal) const {
	// TAREA: Implementar la especificacion del enunciado
	double ret = prevVal + (nextVal - prevVal) * (id - prevId) / (float)(nextId - prevId);
	return ret;
}
